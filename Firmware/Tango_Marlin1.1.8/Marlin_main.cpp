...
uint8_t target_extruder;
  ...
  #define ADJUST_DELTA(V) \
  	if (planner.leveling_active) { \
	   const float zadj = three_point_z_offset(V); \
	   delta[A_AXIS] += zadj; \
	   delta[B_AXIS] += zadj; \
	   delta[C_AXIS] += zadj; \
	 }
...
static void clean_up_after_endstop_or_probe_move() {
  ...
  static float run_z_probe() {
    ...
    // Double-probing does a fast probe followed by a slow probe
      ...
      if (do_probe_move(-100, Z_PROBE_SPEED_FAST)) return NAN;
      ...
      for (uint8_t p = MULTIPLE_PROBING + 1; --p;) {
        ...
        if (do_probe_move(-100, Z_PROBE_SPEED_SLOW)) return NAN;
...
inline void gcode_G4() {
  ...
  inline bool home_delta() {
    ...
    current_position[X_AXIS] = current_position[Y_AXIS] = current_position[Z_AXIS] = (delta_height + 100);
	  ...
	  if(!planner.leveling_active){
	    ...
	    for( x=0;x<GRID_MAX_POINTS_X;x++){
	      for( y=0;y<GRID_MAX_POINTS_Y;y++){
	        if(z_values[x][y]<100.0&&z_values[x][y]>-100.0){}
	        else   return;
	      }
	    }
	  }

	  if(planner.leveling_active !=true){
	    planner.leveling_active=true;
	    refresh_bed_level();
	  }
	  current_position[Z_AXIS] -= bilinear_z_offset(current_position);
	  SYNC_PLAN_POSITION_KINEMATIC();
  #else if ENABLED(AUTO_BED_LEVELING_3POINT)
  float three_point_z_offset(const float raw[XYZ]);
     if(planner.leveling_active ==true){
	  current_position[Z_AXIS] -= three_point_z_offset(current_position);
	  SYNC_PLAN_POSITION_KINEMATIC();
	  }
...
void home_all_axes() { gcode_G28(true); }
   ...
   */
#if ENABLED(AUTO_BED_LEVELING_BILINEAR) 
   float z_get(int i,int j)
   {
      if(z_values[i][j]==999.9)
      {
        if(j<GRID_MAX_POINTS_X/2)
          z_values[i][j]=z_get(i,j+1);
        else if (j>GRID_MAX_POINTS_X/2)
          z_values[i][j]=z_get(i,j-1);
         else
         {
          if(i<GRID_MAX_POINTS_Y/2)
           z_values[i][j]=z_get(i+1,j);
          else if(i>GRID_MAX_POINTS_Y/2)
           z_values[i][j]=z_get(i-1,j);
         }
      }
  ...
  inline void gcode_G29() {
    ...
    #else // !PROBE_MANUALLY
    {
      ...
      measured_z = 0;
		...
		#ifdef ENABLED(AUTO_BED_LEVELING_BILINEAR)
	
		for(int i=0;i<GRID_MAX_POINTS_Y;i++)
		{
			z_get(i,0);
			z_get(i,GRID_MAX_POINTS_Y-1);
		}
		#endif
	   ...
	   SERIAL_PROTOCOLPGM("3Point offset:");
        ...
        for (uint8_t i = 0; i < 3; ++i) {
		  ...
		  SERIAL_PROTOCOL( points[i].z);
	      SERIAL_PROTOCOLPGM("  ");
        }
		
		plane_ratio.a_ratio=( points[0].y*(points[1].z-points[2].z) + points[1].y*(points[2].z-points[0].z) + points[2].y*(points[0].z-points[1].z) ) / ( points[0].y*(points[1].x-points[2].x) + points[1].y*(points[2].x-points[0].x) + points[2].y*(points[0].x-points[1].x));
    
		plane_ratio.b_ratio=( points[0].x*(points[1].z-points[2].z) + points[1].x*(points[2].z-points[0].z) + points[2].x*(points[0].z-points[1].z) ) / ( points[0].x*(points[1].y-points[2].y) + points[1].x*(points[2].y-points[0].y) + points[2].x*(points[0].y-points[1].y));

	    plane_ratio.c_ratio= points[0].z-plane_ratio.a_ratio*points[0].x-plane_ratio.b_ratio*points[0].y;
...
inline void gcode_M502() {
 ...
 /********BIQU ADD************/
        zprobe_baby_stepping = value;
        #if ENABLED(AUTO_BED_LEVELING_BILINEAR) 
        storeBabyStepping();
        #endif
        SERIAL_ECHO(zprobe_baby_stepping);
/*****************************/
    }
    else
     SERIAL_ECHOLNPAIR(": ", zprobe_baby_stepping);
  }
...
void ok_to_send() {
  ...
  void clamp_to_software_endstops(float target[XYZ]) {
    ...
    #define ABL_BG_GRID(X,Y)  (z_values[X][Y]+zprobe_baby_stepping)
  #endif
...
float three_point_z_offset(const float raw[XYZ])
{
	float offset=(plane_ratio.a_ratio*raw[X_AXIS]+plane_ratio.b_ratio*raw[Y_AXIS]+plane_ratio.c_ratio);
	
	return (offset+zprobe_baby_stepping);
}
...
#if ENABLED(POWER_FAILED_SENSOR)
extern bool power_failed_save;
  if(!power_failed_save && READ(POWER_FAILED_PIN)!=POWER_FAILED_INVERTING)
  {
    power_failed_save=true;
    killHeater(PSTR("Power failed!"));
    while(stepper.cleaning_buffer_counter){idle();}
...
void setup() {
  ...
  #if ENABLED(AUTO_BED_LEVELING_BILINEAR) 
   readBabyStepping();
   if(zprobe_baby_stepping>Z_PROBE_OFFSET_RANGE_MIN && zprobe_baby_stepping<Z_PROBE_OFFSET_RANGE_MAX){}
   else zprobe_baby_stepping=0.0;
  #endif
 
#if ENABLED(POWER_FAILED_SENSOR)
  #if POWER_FAILED_INVERTING==true
    SET_INPUT_PULLUP(POWER_FAILED_PIN);
  #else
    SET_INPUT(POWER_FAILED_PIN);
  #endif
#endif
