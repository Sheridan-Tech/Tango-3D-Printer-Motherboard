...
typedef struct
{
	float a_ratio,b_ratio,c_ratio;
}LEVEL_RATIO;
extern LEVEL_RATIO plane_ratio;
#endif
 ...
 */
 ...
 //   return HYPOT2(rx, ry) <= sq(DELTA_PROBEABLE_RADIUS) && HYPOT2(rx - (X_PROBE_OFFSET_FROM_EXTRUDER), ry - (Y_PROBE_OFFSET_FROM_EXTRUDER))<= sq(DELTA_PRINTABLE_RADIUS);
   return position_is_reachable(rx, ry)&&position_is_reachable(rx - (X_PROBE_OFFSET_FROM_EXTRUDER), ry - (Y_PROBE_OFFSET_FROM_EXTRUDER));
  }
