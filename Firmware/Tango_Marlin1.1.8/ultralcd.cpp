...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_z(const float &z) {
    ...
    extern char printFile[LONG_FILENAME_LENGTH];
     ...
     while(stepper.cleaning_buffer_counter){}
	 haltPrinting=true;
...
extern float zprobe_baby_stepping;
#if ENABLED(AUTO_BED_LEVELING_BILINEAR) || ENABLED(AUTO_BED_LEVELING_3POINT) 

void lcd_change_baby_stepping(void) {  
    if (lcd_clicked) {/* storeBabyStepping(); */return lcd_goto_previous_menu(); }
    ENCODER_DIRECTION_NORMAL();
    if (encoderPosition) {
      refresh_cmd_timeout();
     ...
     zprobe_baby_stepping += float((int32_t)encoderPosition) * 0.01;
     
      // Limit only when trying to move towards the limit
      if ((int32_t)encoderPosition < 0) NOLESS(zprobe_baby_stepping, min);
      if ((int32_t)encoderPosition > 0) NOMORE(zprobe_baby_stepping, max);

      encoderPosition = 0;
      lcdDrawUpdate = LCDVIEW_REDRAW_NOW;
    }
    if (lcdDrawUpdate)
      lcd_implementation_drawedit(PSTR("Baby Stepping"),ftostr32(zprobe_baby_stepping));
  }
#endif
...
#if ENABLED(AUTO_BED_LEVELING_BILINEAR) || ENABLED(AUTO_BED_LEVELING_3POINT) 
    MENU_ITEM(submenu, "Baby Stepping", lcd_change_baby_stepping);
#endif

#if ENABLED(POWER_FAILED_SENSOR)
extern bool hasPrintingSys;
if (hasPrintingSys) 
{
    MENU_ITEM(gcode, "To Be Continued...",  PSTR("M996"));
}
#endif
